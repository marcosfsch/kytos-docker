...
ARGS=""
for repo in $(grep ^ARG $DOCKERFILE |  awk -F "[ =]" '{print $2}' | cut -d_ -f2- | sed 's/python_openflow/python-openflow/; s/kytos_utils/kytos-utils/g'); do ARGS="$ARGS --build-arg branch_"$(echo $repo | tr '-' '_')=$(cat /tmp/repo-json-$repo | jq -r '.[].name' | grep $RELEASE | egrep -v -- '-b[0-9]*$' | sort | tail -n1); done
